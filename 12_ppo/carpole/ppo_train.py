...
class PPO:
    def __init__(self, state_dim, hidden_dim, action_dim, actor_lr, critic_lr, lmbda, epochs, eps, gamma, device):
        ...
        self.epochs = epochs                                   # 1条轨迹的数据用于训练的轮次
        self.eps = eps                                          # PPO截断范围参数
        self.device = device
        

    def take_action(self, state):
        state = torch.as_tensor(np.asarray(state), dtype=torch.float32, device=self.device).unsqueeze(0)
...
def main():
    ...
    model_path = Path(__file__).resolve().parent / "ppo_cartpole.pth"
