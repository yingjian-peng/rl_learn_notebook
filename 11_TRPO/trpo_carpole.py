...
class PolicyNet(torch.nn.Module):
    ...
    def forward(self, x):
        x = F.relu(self.fc1(x))
        return F.softmax(self.fc2(x),dim=1)         # [向左概率, 向右概率]
...
class TRPO:
    ...
    def take_action(self, state):
        ...
        probs = self.actor(state)                               # 输出动作概率 [向左概率, 向右概率]
    ...
    def compute_surrogate_obj(self, states, actions, advantage, old_log_probs, actor): 
        log_probs = torch.log(actor(states).gather(1, actions) + 1e-8)          # 当前动作概率
    ...
    def update(self, transition_dict):
        ...
        # 记录策略网络数据
        old_log_probs = torch.log(self.actor(states).gather(1, actions) + 1e-8).detach()    # 提取策略网络的输出动作并隔离梯度更新
        old_action_dists = torch.distributions.Categorical(self.actor(states).detach())     # 创建离散分类分布
...
def main():
    ...
    hidden_dim = 128                                # 隐藏层数
    lmbda = 0.95                                    # 工程中一般就用这个值(控制bias与variance的折中)
    kl_constraint = 0.0005                          # KL散度参数
    alpha = 0.5                                     # 线性搜索缩放系数
    ...
    model_path = Path(__file__).resolve().parent / "trpo_cartpole.pth"
